...
We also include a true retrain robustness block (no checkpoint reuse) across FEMTO, XJTU-SY, and C-MAPSS to validate that conclusions are not purely replay artifacts. A fresh replay sweep on those retrained artifacts still leaves mean width saturated at the \num{125}-step budget, indicating that the external overconservatism is not just a frozen-checkpoint artifact.
...
\section{Limitations}
\begin{itemize}[leftmargin=1.3em]
\item Conservative policy settings can over-prioritize validity margin versus sharpness; in the current canonical external package the mean set width saturates the \num{125}-step budget on FEMTO, XJTU-SY, and C-MAPSS.
\item Replay frontier granularity is still moderate; a denser grid could map the Pareto boundary more precisely.
\item Broader true retrain coverage now reaches FEMTO, XJTU-SY, and C-MAPSS, but external sample size remains intrinsically small on FEMTO/XJTU-SY and the retrained replay sweep still does not recover sharper valid intervals.
...
\section{Conclusion}
The package is audit-ready and manuscript-compilable with artifact-locked results and explicit risk disclosures. Canonical external claims are reproducible and pass integrity checks. Remaining publication risk is primarily reviewer-facing: conservative external widths that persist even after broader true retraining, limited external run counts on FEMTO/XJTU-SY, and replay-grid granularity rather than artifact inconsistency.
