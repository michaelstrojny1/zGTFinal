...
Policy replay keeps model weights and calibration bundles fixed and only changes $(\alpha,\lambda,\text{margin})$. As expected, RMSE remains invariant while validity metrics can shift substantially. In our replay runs, FEMTO validity degrades sharply under less conservative settings, while C-MAPSS remains stable for the same changes.

XJTU-SY replay rows are marked unavailable in non-canonical policies due repeated dataset-download interruption during this session (connection abort on remote transfer). Canonical XJTU-SY remains present and validated in Table~\ref{tab:external-canonical}.
...
\item Non-canonical policy replay is currently incomplete on XJTU-SY in this run due upstream transfer interruptions.
\item Internal readiness score is not a substitute for independent external review.
\end{itemize}

\section{Conclusion}
The package is audit-ready and manuscript-compilable with artifact-locked results and explicit risk disclosures. Canonical external claims are reproducible and pass integrity checks. Remaining publication risk is primarily reviewer-facing (policy conservatism optics and partial non-canonical replay coverage), not artifact consistency.
